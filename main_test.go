...
func TestLandedOrCrashedOnSurface(t *testing.T) {
	...
	if !isLandedOrCrashedOnSurface(s, 3, 0) {
		t.Error()
	}

	s = Surface{0, 0, 4, 4}

	if !isLandedOrCrashedOnSurface(s, 2, 2) {
		t.Error()
	}

	if isLandedOrCrashedOnSurface(s, 2, 3) {
		t.Error()
	}

	s = Surface{0, 0, 3, 2}

	if !isLandedOrCrashedOnSurface(s, 1, 2/3) {
		t.Error()
	}
}

func testMovingCase(t *testing.T, res ShuttleState, expected ShuttleState) {
